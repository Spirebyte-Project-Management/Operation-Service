...
namespace Spirebyte.Services.Operations.Infrastructure.Handlers
{
    public class GenericEventHandler<T> : IEventHandler<T> where T : class, IEvent, ITestEvent
    {
        ...
        public async Task HandleAsync(T @event)
        {
            ...
            var (updated, operation) = await _operationsService.TrySetAsync(correlationId, userId, name, state, null, null, @event.ProjectId, @event.IssueId, @event.SprintId, @event.UserId.ToString());
            if (!updated)
            {
                return;
            }
